...
<html>
    <head>
        ...
        <style>
            ...
            .detailphone{
                ...
                border-radius: 10px;
                padding: 10px;
                margin:10px 15%; /* Căn margin ở trên và dưới để tạo khoảng cách giữa các phần tử */
                text-align: center;
            }
    ...
    <form method="post" action="AddOrderServlet" >
        <div class ="detailphone">
            <c:if test="${empty ms}">
                <img src="${detail.imgProd}" alt="alt"/><br>
            <table>
                <tr>
                    <th>Model</th>
                    <th>Ram</th>
                ...
                <tr>
                    <!-- detail : kiểu trả về Phone -->
                    
                    <td><p>${detail.model}</p></td>
                    <td><p>${detail.ram}</p></td>
                    <td><p>${detail.rom}</p></td>
                    <td><p>${detail.chip}</p></td>
                    <td><p>${detail.battery}</p></td>
                    <td><p>${detail.camera}</p></td>
                    <td><p>${detail.price}$</p></td>
                </tr>
            </table>
            
            <input type="hidden" name="idphone" value="${detail.phoneid}">
            
            <!-- Thêm vào giỏ -->
            <input type="submit"  value="Add to cart" class="button" style="margin-left: 43%"/>
            </c:if>
            
            <!-- Thông báo thành công -->
            <div class="success">
            <c:if test="${not empty ms}">
                <p>You have placed your order successfully</p><br>
                <a href="OrderServlet" class="button" >Order</a>
            </c:if>
            </div>
        </div>
